...
function createUser($conn, $SID, $username, $email, $password, $NID, $designayion, $division,$approval) {
    $sql = "INSERT INTO registration (	sliateId,username,email,password,NID,designation,division,approval) VALUES (?,?,?,?,?,?,?,?);";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
    ...
    mysqli_stmt_bind_param($stmt, "ssssssss",$SID, $username, $email, $hashedpassword, $NID, $designayion, $division,$approval);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
...
function LoginUser($conn, $email, $password) {
    ...
    if ($checkpwd === false){
        ...
        $sql = "INSERT INTO  todaydate(approval) VALUES (1);";
